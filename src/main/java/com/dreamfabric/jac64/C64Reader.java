...
public class C64Reader {
  ...
  private byte sectors[][] = new byte[3000][256];
...
//      int startAddress = adr[1]*256 + adr[0];
  ...
  public boolean readDiskFromFile(String name) {
    try {
      ...
      Path path = Paths.get(f.getPath()+"/c64/carts/"+name);
  ...
  public boolean readPGM(String file, int address) {
    try {
    	 File f = new File(Bukkit.class.getProtectionDomain().getCodeSource().getLocation().toURI()).getParentFile().getParentFile();
         Path path = Paths.get(f.getPath()+"/c64/carts/"+file);
      return readPGM(new FileInputStream(path.toFile()), address);
    } catch (Exception e) {
